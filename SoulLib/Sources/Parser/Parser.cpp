...
namespace Soul::Parser
{
	...
	{
		public:
		typedef NodeIndex (Parser::*PrefixFn)();
		typedef NodeIndex (Parser::*InfixFn)(NodeIndex);
	...
	Parser::Parser(std::string_view module_name, std::string_view script, std::span<const Token> tokens)
		: _tokens(tokens), _current_token(_tokens.begin()), _script(script)
	{
	}

	ASTModule Parser::Parse(std::string_view module_name, std::string_view script, std::span<const Token> tokens)
	{
	...
	ASTModule Parser::Parse()
	{
		...
		for (const auto& token : _tokens) {
			if (token.type == TokenType::TOKEN_SPECIAL_ERROR) {
				_module.nodes.emplace_back(ErrorNode::create(ErrorNode::Message{ token.data }));
			}
		}
		if (!statements.empty()) {
	...
	NodeIndex Parser::ParseStatement()
	{
		// Statements
	...
	NodeIndex Parser::ParseExpression()
	{
	...
	NodeIndex Parser::ParseExpression(Precedence precedence)
	{
	...
	NodeIndex Parser::ParseBinary(NodeIndex lhs)
	{
	...
	NodeIndex Parser::ParseForLoop()
	{
		...
		// [Optional] <expression>
		NodeIndex initialization = nullptr;
	...
	NodeIndex Parser::ParseFunctionCall(NodeIndex dependency)
	{
	...
	NodeIndex Parser::ParseFunctionDeclaration()
	{
	...
	NodeIndex Parser::ParseGrouping()
	{
	...
	NodeIndex Parser::ParseLoopControl()
	...
	NodeIndex Parser::ParseStructDeclaration()
	{
	...
	NodeIndex Parser::ParseVariableDeclaration()
	{
	...
	NodeIndex Parser::ParseWhileLoop()
	{
	...
	NodeIndex Parser::ParseInitializerList()
	{
	...
	NodeIndex Parser::ParseParameterDeclaration()
	{
	...
	NodeIndex Parser::CreateError(Diagnostic diagnostic)
	{
