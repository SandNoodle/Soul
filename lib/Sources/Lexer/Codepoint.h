...
namespace soul
{
	...
	struct Codepoint
	{
		...
		static constexpr bool IsNewline(const ValueType c) noexcept
		{
		...
		static constexpr bool IsWhitespace(const ValueType c) noexcept
		{
		...
		static constexpr bool IsAsciiAlphabetic(const ValueType c) noexcept
		{
		...
		static constexpr bool IsSign(const ValueType c) noexcept
		{
		...
		static constexpr bool IsDigit(const ValueType c) noexcept
		{
		...
		static constexpr bool IsValidNumericContinuator(const ValueType c) noexcept
		{
			static constexpr ValueType k_full_stop = 0x2E;  // U+002E FULL STOP  (.)
			return IsDigit(c) || c == k_full_stop;
		}

		static constexpr bool IsValidStringStarter(const ValueType c) noexcept
		{
		...
		static constexpr bool IsValidIdentifierStarter(const ValueType c) noexcept
		{
			static constexpr ValueType k_low_line = 0x5F;  // U+005F LOW LINE (_)
			return IsAsciiAlphabetic(c) || c == k_low_line;
		}

		static constexpr bool IsValidIdentifierContinuator(const ValueType c)
		{
