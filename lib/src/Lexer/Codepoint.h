...
namespace soul
{
	...
	struct Codepoint
	{
		...
		static constexpr bool IsNewline(ValueType c) noexcept
		{
		...
		static constexpr bool IsWhitespace(ValueType c) noexcept
		{
		...
		static constexpr bool IsAsciiAlphabetic(ValueType c) noexcept
		{
		...
		static constexpr bool IsDigit(ValueType c) noexcept
		{
			static constexpr ValueType k_zero = 0x30;  // U+0030 Digit Zero (0)
			static constexpr ValueType k_nine = 0x39;  // U+0030 Digit Nine (9)
			return c >= k_zero && c <= k_nine;
		}

		static constexpr bool IsIdentifier(ValueType c) noexcept
		{
