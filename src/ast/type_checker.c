...
// @TODO: Implement panic mode for the type_checker so we can continue type checking before returning an error.
...
static soul_basic_type_t soul_type_checker_compare(soul_basic_type_t first, soul_basic_type_t second)
{
	// @TODO
	return soul_basic_type_unknown;
}

static soul_basic_type_t soul_type_checker_get_type(soul_type_checker_t* type_checker, soul_ast_node_identifier_t id)
{
	// @TODO
	return soul_basic_type_unknown;
}

static soul_basic_type_t soul_type_checker_traverse(soul_type_checker_t* type_checker, soul_ast_node_t* node)
{
	switch(node->type)
	{
		case soul_ast_expr_assign: {
			soul_basic_type_t lhs = soul_type_checker_traverse(type_checker, node->as.expr_assign.lhs);
			soul_basic_type_t rhs = soul_type_checker_traverse(type_checker, node->as.expr_assign.rhs);
			return soul_type_checker_compare(lhs, rhs);
		}
		case soul_ast_expr_unary:
			return soul_type_checker_traverse(type_checker, node->as.expr_unary.expr);
		case soul_ast_expr_binary: {
			soul_basic_type_t lhs = soul_type_checker_traverse(type_checker, node->as.expr_binary.lhs);
			soul_basic_type_t rhs = soul_type_checker_traverse(type_checker, node->as.expr_binary.rhs);
			return soul_type_checker_compare(lhs, rhs);
		}
		case soul_ast_expr_var_literal:
			return soul_type_checker_get_type(type_checker, node->as.expr_literal_variable.id);
		...
		case soul_ast_expr_stmt:
			return soul_type_checker_traverse(type_checker, node->as.expr_stmt.stmt);
		case soul_ast_stmt_if: {
			soul_basic_type_t condition_type = soul_type_checker_traverse(type_checker, node->as.stmt_if.condition);
			if(condition_type != soul_basic_type_bool)
				return soul_basic_type_unknown;
			soul_basic_type_t then_body_type = soul_type_checker_traverse(type_checker, node->as.stmt_if.then_body);
			if(then_body_type == soul_basic_type_unknown)
				return then_body_type;
			if(node->as.stmt_if.else_body)
			{
				soul_basic_type_t else_body_type = soul_type_checker_traverse(type_checker, node->as.stmt_if.else_body);
		...
		case soul_ast_stmt_block: {
			...
			{
				soul_basic_type_t result = soul_type_checker_traverse(type_checker, stmts->nodes[i]);
				if(result == soul_basic_type_unknown)
					return result;
			}
		}
...
soul_type_checker_result_t soul_type_checker_analyze(soul_type_checker_t* type_checker, soul_ast_node_t* root)
{
	soul_basic_type_t result_type = soul_type_checker_traverse(type_checker, root);
	if (result_type == soul_basic_type_unknown) {
		const char* error_message = "todo: type checking failed. this should return a boolean and error message should be a callback instead.";
...
soul_basic_type_t cast_to_common_type(soul_basic_type_t from, soul_basic_type_t to)
{
...
soul_basic_type_t type_checker_compare_types(soul_basic_type_t first, soul_basic_type_t second)
{
	if (first == second)
		return first;
