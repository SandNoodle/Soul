...
static soul_token_t lexer_scan_token(soul_lexer_t* lexer)
{
	...
	switch(c)
	{
		...
		#define MAKE_MULTI_CHAR_TOKEN(next_char, type_match, type_not_match) \
			return lexer_create_token(lexer, lexer_match_next(lexer, (next_char)) ? (type_match) : (type_not_match))
...
static soul_token_t lexer_create_token(soul_lexer_t* lexer, soul_token_type_t type)
{
	soul_token_t token = {
		.type = type,
...
static soul_token_t lexer_create_error_token(soul_lexer_t* lexer, const char* message)
{
	soul_token_t token = {
...
static soul_token_t lexer_create_identifier_token(soul_lexer_t* lexer)
{
	...
	for(size_t index = 0; index < soul_token_keywords_size; ++index)
	{
		...
		bool same_string = memcmp(lexer->token_start, keyword.start, length) == 0;
...
static char lexer_peek(soul_lexer_t* lexer)
{
	return *lexer->current_char;
}
